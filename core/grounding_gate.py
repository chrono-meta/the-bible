...
def scripture_grounded(quote: str, ref: str) -> bool:
    ...
    (normalized) contains the quoted text. Multi-version union: a quote verified in ANY
    tradition (Protestant KJV/WEB/ASV/YLT · Catholic Douay-Rheims · Apocrypha KJVA) counts.
    No verified match in any version -> not grounded -> fail-closed."""
    nq = _norm(quote)
    for ver in SCRIPTURE_VERSIONS.values():
        text = ver.get(ref)
        if text and nq in _norm(text):
            return True
    return False
