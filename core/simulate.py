#!/usr/bin/env python3
"""
The Bible — persona simulation (입장 시뮬).
...
def run():
    print("=== The Bible — 페르소나 입장 시뮬레이션 ===")
    print("(성서=절대 axiom·검증 안 함 / AI=순수 매개·검증 성구만 / 위기>위안 / 사죄 안 함)\n")
    for p in PERSONAS:
        resp = p["response"][0] if isinstance(p["response"], tuple) else p["response"]
        r = gate(p["input"], resp, p["citations"])
        print(f"{p['name']}")
        print(f"  입장 발화: {p['input']}")
        print(f"  ── 게이트 판정: {r['verdict']}")
        if r["verdict"] == "PASS":
            print("  ── 하네스 응답 (성서 grounding 통과):")
            for line in resp.splitlines():
                print(f"     {line}")
        elif r["verdict"] == "CRISIS_OVERRIDE":
            print("  ── 위기 override (위안·폐기보다 우선):")
            for line in r["output"].splitlines():
                print(f"     {line}")
            print(f"     actions: {r['actions']}")
        elif r["verdict"] == "FAIL_CLOSED":
            print(f"  ── 차단 (검증 안 된 성구 인용): {r['ungrounded']}")
            print(f"     {r['output']}")
        print()
